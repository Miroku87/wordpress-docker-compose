...
class Redeemable_Codes_Admin
{
	...
	{
		require_once plugin_dir_path(__FILE__) . 'partials/multiverso-leaderboard-admin-display-codes.php';
	}
	...
	{
		require_once plugin_dir_path(__FILE__) . 'partials/multiverso-leaderboard-admin-display-cors.php';
	}
	...
	private function handle_codes_form_submission()
	{
		...
		if (isset($_POST['speedtale_id']) && $_POST['speedtale_id'] != '') {
			...
			add_settings_error('multiverso-leaderboard-notices', 'multiverso-leaderboard-notices', __('Missing or invalid SpeedTale ID.', $this->plugin_name), 'error');
			return;
		}
		...
		if (isset($_POST['item']) && $_POST['item'] != '') {
			...
			add_settings_error('multiverso-leaderboard-notices', 'multiverso-leaderboard-notices', __('Missing item name.', $this->plugin_name), 'error');
			return;
		}
		...
		switch ($code_type) {
			...
			default:
				...
				add_settings_error('multiverso-leaderboard-notices', 'multiverso-leaderboard-notices', $message, 'error');
		}
	}
	...
	private function handle_random_code_submission($speedtale_id, $item, $expiration_days)
	{
		if (isset($_POST['number_of_codes']) && intval($_POST['number_of_codes']) > 0) {
			...
			add_settings_error('multiverso-leaderboard-notices', 'multiverso-leaderboard-notices', __('Invalid number of codes.', $this->plugin_name), 'error');
			return;
		}
		...
		if (count($generated_codes) > 0) {
			...
			add_settings_error('multiverso-leaderboard-notices', 'multiverso-leaderboard-notices', $message, 'updated');
			return;
		}

		add_settings_error('multiverso-leaderboard-notices', 'multiverso-leaderboard-notices', __('No codes generated.', $this->plugin_name), 'error');
	}
	...
	private function handle_custom_code_submission($speedtale_id, $item, $expiration_days)
	{
		if (isset($_POST['custom_code']) && $_POST['custom_code'] != '') {
			...
			add_settings_error('multiverso-leaderboard-notices', 'multiverso-leaderboard-notices', __('Missing or invalid code.', $this->plugin_name), 'error');
			return;
		}
		...
		if ($is_unique && !is_wp_error($codeInDb)) {
			add_settings_error('multiverso-leaderboard-notices', 'multiverso-leaderboard-notices', __('Code should be unique but already exists in the database.', $this->plugin_name), 'error');
			return;
		}

		if (!$is_unique && !is_wp_error($codeInDb) && $codeInDb->is_unique == 1) {
			add_settings_error('multiverso-leaderboard-notices', 'multiverso-leaderboard-notices', __('Code already exists in the database and is set as unique.', $this->plugin_name), 'error');
			return;
		}
	...
	private function handle_code_delete_request()
	{
		...
		if ($code_id <= 0) {
			add_settings_error('multiverso-leaderboard-notices', 'multiverso-leaderboard-notices', __('Invalid code ID.', $this->plugin_name), 'error');
		}
		...
		add_settings_error('multiverso-leaderboard-notices', 'multiverso-leaderboard-notices', __('Code deleted.', $this->plugin_name), 'updated');
	}
	...
	private function get_redeemable_code_info($request)
	{
		...
		if (!isset($code) || !isset($speedtale_id)) {
			return new WP_Error('multiverso-leaderboard-error', __('Invalid request.', $this->plugin_name), array('status' => 400));
		}
	...
	{
		register_rest_route('multiverso-leaderboard/v1', '/codes/(?P<code>[a-zA-Z0-9-]+)/redeem', array(
			'methods' => 'PATCH',
