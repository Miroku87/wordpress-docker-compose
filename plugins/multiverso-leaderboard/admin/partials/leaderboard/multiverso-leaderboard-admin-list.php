...
global $wpdb;
$table_name = $wpdb->prefix . MULTIVERSO_LB_TABLE_NAME;
$leaderboard = $wpdb->get_results("SELECT * FROM $table_name ORDER BY id DESC");
?>
<!-- Record della Classifica -->
<h2><?php esc_html_e('Record della Classifica', $this->plugin_name); ?></h2>

<?php if (count($cors_origins) == 0) : ?>
    <p>
        <?php esc_html_e('Nessun record inserito.', $this->plugin_name); ?>
    </p>
<?php else : ?>
